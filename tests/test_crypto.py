...
def test_secret_hashing_is_versioned_salted_and_legacy_compatible():
    ...
    assert int(stored.split("$")[1]) == PBKDF2_ITERATIONS
...
def test_passphrase_aes_gcm_envelope_detects_tampering():
    ...
    assert metadata["kdf_iterations"] == PBKDF2_ITERATIONS
    ...
    hostile = {**metadata, "kdf_iterations": MAX_PBKDF2_ITERATIONS + 1}
    with pytest.raises(ValidationError, match="iterations"):
        decrypt_bytes_with_passphrase(cipher, "shared passphrase", hostile)
    assert not verify_secret_hash(
        "value", f"pbkdf2_sha256${MAX_PBKDF2_ITERATIONS + 1}$00$00"
    )
